...
function init(){
    ...
    // 上面实现了overlay的模块，下面我们来实现热力图模块。
    // 话不多说，先来实现一个heatmap图层先
    let vector_heat = new ol.layer.Heatmap({
        source: new ol.source.Vector({
            projection: 'EPSG:3857',
            url:"../data/city.json",
            format: new ol.format.GeoJSON()
        }),
        // blur 和radius都先设置成固定样式，后续考虑可动
        blur:10,
        radius:10,
    });

    vector_heat.getSource().on('addfeature', function(event) {
        $.ajax({
            url:`/getdata/${event.feature.get('en-name')}`,
            type:'get',
            success:function(data){
                var data2 = document.getElementById('chinaData');
                var get_loc = data2.getAttribute("value");
                var loc = `${get_loc}`;

                weight_out = data['Confirmed']/loc * 10;
                // weight_out = Math.log10(data['Confirmed']);

                event.feature.set('weight', weight_out);

            }
        }); 
        event.feature.set('weight', 0.5);  
      });

    map.addLayer(vector_heat);
};
